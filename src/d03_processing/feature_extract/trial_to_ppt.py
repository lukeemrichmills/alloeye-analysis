...
def trial_to_ppt(ppts, via_timepoints=True, timepoints=None):
    """"""
    ...
    try:
        ...
        summed_df = summing_df.groupby(['ppt_id'], as_index=False).sum()
        ...
        aved_df = av_df.groupby(['ppt_id'], as_index=False).mean()
        ...
        cond_corr_df, new_cols = feature_per_condition(corr_df, 'correct', groupby='ppt_id', groupfunc='proportion')
        cond_corr_df_long = cond_corr_df.reset_index().melt(id_vars='ppt_id', value_vars=new_cols,
        ...
        for feature in av_df.columns.values:
            ...
            if feature != 'ppt_id':
                ...
                cond_df, new_cols = feature_per_condition(aving_df, feature, groupby='ppt_id', groupfunc='mean')
                cond_df = cond_df.drop([feature], axis=1)
                new_name = f'{feature}'
                cond_df_long = cond_df.reset_index().melt(id_vars='ppt_id', value_vars=new_cols,
                                                          var_name='condition', value_name=new_name)
                full_cond_df = pd.concat([full_cond_df, cond_df_long.drop(['ppt_id', 'condition'], axis=1)], axis=1)
        full_cond_df = add_col_by_lookup(full_cond_df, 'group', 'ppt_id', pid_info, 'alloeye_id', 'group')
    except:
        print("error - save dfs first")
